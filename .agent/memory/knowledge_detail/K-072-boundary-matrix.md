# K-072 — Shared Boundary Matrix (Fase 0)

**Categoria:** Matriz de decisão de fronteiras entre packages por fase.

## Matriz de Decisao

| Grupo de Codigo | Destino | Permitido Agora? | Motivo | Dono da Decisao | Fase de Implementacao |
|-----------------|---------|------------------|--------|-----------------|----------------------|
| **Schemas Zod** | `packages/core` | NAO (Fase 2) | Fase 0 = guardrails, Fase 2 = migracao | ADR-026 | Fase 2 |
...
| **Componentes React** | FICAR em `src/shared/components/` | **SIM** | Nao podem rodar em React Native; web-only | ADR-026 | — |
...
- Web importa tipos via `import { medicineSchema } from '@meus-remedios/core'`
...
- Web importa schemas de `@meus-remedios/core` (rollback strategy em Fase 3)
...
|------|------------------|------------|---------------|
| Schemas | `packages/core/schemas/` | `@meus-remedios/core` | `@meus-remedios/core` |
| Utils puros | `packages/core/utils/` | `@meus-remedios/core` | `@meus-remedios/core` |
...
**R:** ADR-027 explica — um usuario pode ter multiplos devices. Tabela dedicada permite auditoria e revogacao clara.

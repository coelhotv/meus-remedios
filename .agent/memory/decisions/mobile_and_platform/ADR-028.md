---
id: ADR-028
title: StyleSheet + AsyncStorage no MVP (Fase 4-5)
status: accepted
date: 2026-04-10
tags:
  - mobile
  ...
  - styling
  - storage
context: MVP mobile (Phases 4-5) must run on React Native 0.75+ with Expo. Multiple styling and storage alternatives available.
decision: Use React Native StyleSheet (not NativeWind) and AsyncStorage (not MMKV/SQLite) to minimize native module friction. Migration to advanced options allowed only in Phase 8+.
source_pr: 457
supersedes: None
superseded_by: None
---

# ADR-028 — StyleSheet + AsyncStorage no MVP (Fase 4-5)

...
**Entrada base:** `plans/archive_old/stock_refactor/VISION_STOCK_PURCHASES_REFACTOR.md`
...
## 21. Regressões identificadas em produção (pós-entrega)

### 21.1 Phantom stock — `consume_stock_fifo` com filtro entry_type incorreto

**Data:** 2026-05-13  
**Descoberta:** auditoria via MCP Supabase  
**AP registrado:** AP-142  
**Regra registrada:** R-226

#### Problema

A implementação de `consume_stock_fifo` adicionou `AND entry_type = 'purchase'` no loop FIFO e no pre-check de disponibilidade. Este filtro **não está na spec** (seção 7.2, pseudocódigo).

Consequência: entradas `entry_type = 'adjustment'` com `quantity > 0` são incluídas em `medicine_stock_summary` (view não filtra entry_type), mas são invisíveis para o FIFO. O usuário vê saldo positivo na UI mas recebe `Estoque insuficiente` ao registrar dose.

Medicamentos afetados na auditoria:

| Medicamento | FIFO consumível | UI mostrava |
|---|---|---|
| SeloZok | 0 | 4 (bloqueado) |
| Clopidogrel | 58 | 60 |
| Olmesartana | 55 | 58 |
| Ácido Acetilsalicílico | 10 | 13 |

As entradas `adjustment` com saldo positivo são pílulas reais — criadas pelo sistema legado ao excluir doses (backfill as classificou como `adjustment`).
...
Aplicado em ambos os overloads: `consume_stock_fifo(UUID, NUMERIC, UUID)` e `consume_stock_fifo(UUID, UUID, NUMERIC, UUID)`.

SeloZok phantom stock zerado manualmente com auditoria em `stock_adjustments` (`reason = 'ajuste_manual_legacy'`).

#### Correção da spec (seção 7.2)

O pseudocódigo da seção 7.2 deve ser lido com a seguinte cláusula WHERE **obrigatória**:
...
WHERE user_id = auth.uid()
  AND medicine_id = p_medicine_id
  AND quantity > 0
  AND entry_type != 'legacy_unrecoverable'  -- ← filtro correto
ORDER BY purchase_date ASC, created_at ASC, id ASC
FOR UPDATE
```

...
export default function StreakBadge({ streak = 0, size = 'md', showLabel = true, className = '' }) {
  ...
  return (
    <div
      className={`streak-badge streak-badge-${size} ${isHighStreak ? 'high-streak' : ''} ${className}`}
    >
      <span className="streak-icon">{isHighStreak ? '🔥' : '✓'}</span>
      <span className="streak-count">{streak}</span>
      {showLabel && <span className="streak-label">{streak === 1 ? 'dia' : 'dias'}</span>}
    </div>
  )
}
